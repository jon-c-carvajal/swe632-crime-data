        ...
        .on("mouseover", function(d) {
            if (!d3.select(this).classed("invisible-shape")) {
                div.transition()
                    .duration(1)
                    .style("opacity", .9);
                var str = display_name + "<br/>Year : " + d[[x_name]] + "<br/> Count : " + d[[y_name]];
                if(d["numGuns"]){
                    str += "<br/> # of Guns : " + d["numGuns"];
                }
...
function crimePlot(svgId, estimates, stateMapInfo, title, plotEntries, numTicksX) {
    ...
    if (svgId == "#state-content10" || svgId == "#state-content11") {
        //not in d3.v3
        //var parseTime = d3.timeParse("%d-%b-%y");
        var gunX = d3.scaleLinear().range([0, width]);
        var gunY = d3.scaleLinear().range([height, 0]);

        gunY.domain([0, 1.3 * d3.max(estimates, d => d["numGuns"])]);
        gunX.domain(d3.extent(estimates, function(d) { return d["year"]; }));

        var line = d3.area()
            .x(function(d) { return gunX(d["year"]); })
            .y0(height)
            .y1(function(d) {
                return gunY(d["numGuns"]);
            })
